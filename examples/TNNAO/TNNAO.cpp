...
class TNNAOApplication : public kbs::Application
{
public:

	TNNAOApplication(kbs::ApplicationCommandLine& commandLine)
...
namespace kbs
{
	...
	{
		auto app = new TNNAOApplication(commandLines);
		return app;
	}
}

ptr<Scene> scene;
ptr<TNNAORenderer>	renderer;
...
void TNNAOApplication::BeforeRun()
{
	scene = std::make_shared<Scene>();
	renderer = std::make_shared<TNNAORenderer>();


	Singleton::GetInstance<FileSystem<ShaderManager>>()->AddSearchPath(TNNAO_ROOT_DIRECTORY);
		...
		{
			{
				...
				env.intensity = kbs::vec3(1.0, 1.0, 1.0) * 0.8f;
...
void TNNAOApplication::OnUpdate()
{
	{
